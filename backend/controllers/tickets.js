...
exports.handleTicket = async (req, res, next) => {
    ...
    existingTicket.etat = IN_PROGRESS;
    existingTicket.treatedBy = {
        full_name: existingUser.full_name,
        _id: existingUser._id,
        role: existingUser.role,
        email: existingUser.email
    ...
    existingTicket.save()
        ...
        .catch(error => {
            res.status(500).json({
                message: "Couldn't Reserve Ticket!"
            });
        });
};
...
exports.resolveTicket = async (req, res, next) => {
    ...
    existingTicket.remarques = req.body.remarques,
        existingTicket.filePath = filePath,
        existingTicket.etat = FINISHED
    ...
    existingTicket.save()
        ...
        .catch(error => {
            res.status(500).json({
                message: "Couldn't resolve Ticket!"
            });
        });
};
